...
const TeacherRequest = () => {
 ...
 const acceptMutation = useMutation({
  mutationFn: (id) =>
    axiosSecure.patch(`/teacher-requests/approve/${id}`, { status: "approved" }),
  onSuccess: () => {
    Swal.fire({
      icon: "success",
      title: "Approved!",
...
const rejectMutation = useMutation({
  mutationFn: (id) =>
    axiosSecure.patch(`/teacher-requests/reject/${id}`, { status: "rejected" }),
  onSuccess: () => {
    Swal.fire({
      icon: "success",
      title: "Rejected!",
          ...
          <tbody>
            {data.map((req) => (
              <tr key={req._id}>
                <td>{req.name}</td>
                <td>
                  <img src={req.image} alt="img" className="w-12 h-12 rounded" />
                </td>
                ...
                <td className="space-x-2">
                  <button
                    ...
                    disabled={req.status !== "pending" || acceptMutation.isLoading}
                  ...
                  <button
                    ...
                    disabled={req.status === "rejected" || rejectMutation.isLoading}
